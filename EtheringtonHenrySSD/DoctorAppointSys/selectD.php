...
<html lang='en'>
  <head>
    <title>Select Doctor</title>
     ...
     <link rel="stylesheet" type="text/css" href="selectD.css">
    <body>
...
$sql = 'SELECT count(firstName) FROM doctor where firstName = :cfirstName';
$result = $pdo->prepare($sql);
$result->bindValue(':cfirstName', $_POST['firstName']); 
$result->execute();
if($result->fetchColumn() > 0) 
{
    $sql = 'SELECT firstName FROM doctor where firstName = :cfirstName';
    $result = $pdo->prepare($sql);
    $result->bindValue(':cfirstName', $_POST['firstName']); 
    $result->execute();
